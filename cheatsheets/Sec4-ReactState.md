...
Basically whenever we make a two-way binding we are controlling a component, mostly our custom components. In these cases, the child component *i.e. ExpensesFilter* is called as **controlled component** since we set the values in the parent component. Moreover a component that has *state* in it called as **smart/stateful component**. And the ones without a state is called as **dump/stateless component**. At the end of a project, it is typical that we end up in having more dump components than smart components. So the state is used just in some comopnents and is spreaded around the other ones and used.